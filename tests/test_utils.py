...
class TestUtils(unittest.TestCase):
    ...
    def test_getCommunityEdge(self):
        ...
        for a in filelist:
            ...
            source_node_name_list = [value['n']
                                     for key, value in list(testcx.get_nodes())]
            node_id_list = []
            for a in g.vs['name']:
            ...
            v_community = [int(testcx.get_node_attribute_value(
                a, 'community')) for a in node_id_list]

            source_edge_list = [(node_id_list.index(value['s']), node_id_list.index(
                value['t'])) for key, value in list(testcx.get_edges())]
            ...
            e_community = [int(testcx.get_edge_attribute_value(
                a, 'community')) for a in edge_id_list]

            self.assertEqual(
                e_community, agviz.utils.getCommunityEdge(g, v_community))
    ...
    def test_communityToColors(self):
        ...
        for a in filelist:
            ...
            self.assertEqual(
                v_color, agviz.utils.communityToColors('hls', v_community))
            self.assertEqual(
                e_color, agviz.utils.communityToColors('hls', e_community))
